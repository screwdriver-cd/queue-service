...
describe('Plugin Test', () => {
    ...
    beforeEach(() => {
        ...
        BlockedBy = require('../../../../plugins/worker/lib/BlockedBy')
            .BlockedBy;
    ...
    describe('BlockedBy', () => {
        ...
        describe('beforePerform', () => {
            beforeEach(() => {
                ...
                mockRedis.get
                    .withArgs(`${runningJobsPrefix}111`)
                    .resolves(null);
                mockRedis.get
                    .withArgs(`${runningJobsPrefix}222`)
                    .resolves(null);
            ...
            it('do not proceed if build was aborted while running', async () => {
                mockRedis.get.withArgs(deleteKey).resolves('');
                mockRedis.get
                    .withArgs(`${runningJobsPrefix}${jobId}`)
                    .resolves('4');
                const result = await blockedBy.beforePerform();
            ...
            it('re-enqueue if blocked', async () => {
                ...
                assert.equal(
                    mockRedis.get.getCall(2).args[0],
                    `last_${runningKey}`
                );
                assert.equal(
                    mockRedis.get.getCall(3).args[0],
                    `${runningJobsPrefix}111`
                );
                assert.equal(
                    mockRedis.get.getCall(4).args[0],
                    `${runningJobsPrefix}222`
                );
                assert.notCalled(mockRedis.set);
                assert.notCalled(mockRedis.expire);
                assert.calledWith(
            ...
            it('re-enqueue if blocked multiple build ids', async () => {
                ...
                assert.equal(
                    mockRedis.get.getCall(5).args[0],
                    `${runningJobsPrefix}777`
                );
                assert.notCalled(mockRedis.set);
                assert.notCalled(mockRedis.expire);
                assert.calledWith(
            ...
            it('do not re-enqueue if build from the same event id', async () => {
                ...
                mockRedis.hget
                    .withArgs(`${queuePrefix}buildConfigs`, buildId)
                    .resolves(JSON.stringify(buildConfig1));
                mockRedis.hget
                    .withArgs(`${queuePrefix}buildConfigs`, '4')
                    .resolves(JSON.stringify(buildConfig2));
                mockRedis.hget
                    .withArgs(`${queuePrefix}buildConfigs`, '5')
